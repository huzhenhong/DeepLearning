...
), "version of tf must greater/equal than 2.4.0"


def main():
    # save pytorch weights path
    save_path = "./pre_train_model/efficientnetb0.pth"
    ...
    for weight in weights:
        ...
        if "stem_conv/kernel:0" == name:
            ...
            weights_dict[torch_name] = np.transpose(data, (3, 2, 0, 1)).astype(
                np.float32
            )
            ...
            trans_dict = {
                ...
                "se_reduce/kernel:0": "se.fc.0.weight",  # "se.fc1.weight",
                "se_reduce/bias:0": "se.fc.0.bias",  # "se.fc1.bias",
                "se_expand/kernel:0": "se.fc.2.weight",  # "se.fc2.weight",
                "se_expand/bias:0": "se.fc.2.bias",  # "se.fc2.bias",
            ...
            if torch_postfix in [
                "expand_conv.0.weight",
                "se.fc.0.weight",
                "se.fc.2.weight",
                "project_conv.0.weight",
            ]:  # ["expand_conv.0.weight", "se.fc1.weight", "se.fc2.weight", "project_conv.0.weight"]:
            ...
            weights_dict[torch_name] = np.transpose(data, (1, 0)).astype(
                np.float32
            )
