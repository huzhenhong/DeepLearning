...
def read_split_data(data_root, save_dir, val_rate=0.2, plot_image=False):
    ...
    # 生成类别名称以及对应的数字索引
    class_indices = dict((k, v) for v, k in enumerate(mnist_classes))
    json_str = json.dumps(
    ...
    if plot_image:
        # 绘制每种类别个数柱状图
        plt.bar(range(len(mnist_classes)), every_class_num, align='center')
        # 将横坐标0,1,2,3,4替换为相应的类别名称
        plt.xticks(range(len(mnist_classes)), mnist_classes)
        # 在柱状图上添加数值标签
        ...
        plt.savefig(os.path.join(save_dir, "mnist_classes.jpg"))
        # plt.show()
    return (
        train_images_path,
        val_images_path,
...
if __name__ == '__main__':
    read_split_data(
        data_root=r'D:\dataset\MNIST_Dataset\train_images', save_dir='./'
    )
