    ...
    def __init__(
        ...
        if use_post_se:
            self.post_se = SEBlock(
                out_channels, internal_neurons=out_channels // 4
            )
        else:
...
class RepVGGplus(nn.Module):
    ...
    def _build_aux_for_stage(self, stage):
        stage_out_channels = list(stage.blocks.children())[
            -1
        ].rbr_dense.conv.out_channels
        ...
        return nn.Sequential(
            downsample, nn.AdaptiveAvgPool2d(1), nn.Flatten(), fc
        )
...
def create_RepVGGplus_L2pse(
    deploy=False, use_checkpoint=False, num_classes=1000
):
    return RepVGGplus(
