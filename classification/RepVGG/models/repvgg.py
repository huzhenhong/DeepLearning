...
class RepVGG(nn.Module):
    def __init__(
        ...
        # 通道缩放倍数，模型有5个stage，stage1和stage2共用第一个width_multiplier
        assert len(width_multiplier) == 4

        # 训练或者推理
        self.deploy = deploy
        # 组卷积。字典类型，key为layer的索引，value为组卷积的参数group
        ...
        # 搭建stage0，由repvgg block组成，下采样一倍 112->56.
        ...
        # 当前层索引，用于self.override_groups_map
        self.cur_layer_idx = 1
        # 每个stage的第一个block进行下采样，随后每个stage剩余的block步长=1
    ...
    def _make_stage(self, planes, num_blocks, stride):
        # 计算每个block的stride，除了每个stage的第一个block进行下采样，其余block的stride都为1
        strides = [stride] + [1] * (num_blocks - 1)
        blocks = []
        # 遍历stride,有几个stride就有几个block
        for stride in strides:
            # dict.get(a,b)。a为key值，若a存在，返回dict[a]，若不存在，返回b
