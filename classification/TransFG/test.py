...
def run(
    ...
    model = build_model(config, num_classes=config["train"]["num_classes"])

    model.load_state_dict(
        torch.load(weights, map_location=device)["model"], strict=True
    )
    model.eval().to(device)

    # run once
    y = model(torch.rand(1, 3, 448, 448).to(device))
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--weights', type=str, default='best.pth', help='the model path'
    )
    parser.add_argument(
