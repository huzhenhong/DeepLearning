...
def parse_option():
    parser = argparse.ArgumentParser(
        'GoogLeNet training and evaluation script', add_help=False
    )
    parser.add_argument(
        '--cfg',
        type=str,
...
def main(config):
    ...
    if (
        ...
        and (not config.MODEL.RESUME)
        and config.MODEL.GOOGLENET.AUX_LOGITS
    ):
...
def train_one_epoch(
    ...
    for idx, (samples, targets) in enumerate(data_loader):
        ...
        if config.MODEL.GOOGLENET.AUX_LOGITS:
            loss = (
                criterion(outputs.logits, targets)
                + criterion(outputs.aux_logits2, targets)
                + criterion(outputs.aux_logits1, targets)
            )
        else:
