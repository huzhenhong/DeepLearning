...
class RectLabelConverter(BaseLabelConverter):
    def custom_to_voc2017(self, input_file, output_dir):
        ...
        ET.SubElement(size, 'depth').text = '3' # color image
        ...
        with open(output_dir, 'w') as f: # output_dir ?
    ...
    def coco_to_custom(self, input_file, output_path, image_path):
        ...
        for dic_info in tqdm(
            ...
            output_file = os.path.join(
                output_path,
                os.path.splitext(dic_info["imagePath"])[0] + ".json",
            )
            with open(output_file, 'w', encoding='utf-8') as f:
                json.dump(self.custom_data, f, indent=2, ensure_ascii=False)
    ...
    if args.mode == "custom2voc":
        ...
        for file_name in tqdm(
            ...
            dst_file = os.path.join(
                args.dst_path, os.path.splitext(file_name)[0] + '.xml'
            )
        ...
        for file_name in tqdm(
            ...
            src_file = os.path.join(args.src_path, file_name)
            dst_file = os.path.join(
                args.dst_path, os.path.splitext(file_name)[0] + '.txt'
            )
        ...
        for file_name in tqdm(
            ...
            img_file = os.path.join(
                args.img_path, img_dic[os.path.splitext(file_name)[0]]
            )
