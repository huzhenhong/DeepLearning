...
def main(args):
    ...
    if os.path.exists(args.weights):
        ...
        model.load_state_dict(
            load_weights_dict, strict=False
        )  # strict=False,不完全匹配
    else:
    ...
    lf = (
        lambda x: ((1 + math.cos(x * math.pi / args.epochs)) / 2)
        * (1 - args.lrf)
        + args.lrf
    )  # cosine
    ...
    for epoch in range(args.epochs):
        ...
        print(
            "[epoch {}] train accuracy: {}".format(epoch, round(train_acc, 3))
        )
        ...
        if fig is not None:
            tb_writer.add_figure(
                "predictions vs. actuals", figure=fig, global_step=epoch
            )

        # add conv1 weights into tensorboard
        tb_writer.add_histogram(
            tag="conv1", values=model.conv1.weight, global_step=epoch
        )
        tb_writer.add_histogram(
            tag="layer1/block0/conv1",
        ...
        if val_loss > best_acc:
            ...
            best_acc = val_acc
            shutil.copy(
                "./weights/model-{}.pth".format(best_epoch),
                "./weights/best_mdoel.pth",
            )
