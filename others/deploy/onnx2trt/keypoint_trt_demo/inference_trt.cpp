...
static const int EXPLICIT_BATCH =
    1 << (int)(NetworkDefinitionCreationFlag::kEXPLICIT_BATCH);
...
void doInference(IExecutionContext &context, float *input, float *output,
  ...
  CHECK(cudaMalloc(&buffers[inputIndex],
                   batchSize * 3 * IN_H * IN_W * sizeof(float)));
  // CHECK(cudaMalloc(&buffers[outputIndex], batchSize * 3 * IN_H * IN_W / 4 *
  // sizeof(float)));
  ...
  // CHECK(cudaMemcpyAsync(output, buffers[outputIndex], batchSize * 3 * IN_H *
  // IN_W / 4 * sizeof(float), cudaMemcpyDeviceToHost, stream));
  CHECK(cudaMemcpyAsync(output, buffers[outputIndex],
                        batchSize * 1000 * sizeof(float),
